...
import { ScrollView, View, TouchableOpacity, Alert, Modal, FlatList, ActivityIndicator, TextInput, Switch } from 'react-native';
...
export default function PartnerRegisterIndividualScreen() {
  ...
  const onSubmit = async (data: IndividualPartnerForm) => {
    // Validate ảnh: CCCD/CMND trước/sau, ít nhất 3 ảnh gallery
    if (!idFront || !idBack) {
      Alert.alert(
    ...
    try {
      setLoading(true);
      // 1. Upload tất cả file lên backend /api/file/upload để lấy file_path
      ...
      const idFrontPath = await uploadSingle(idFront, 1);
      filesPayload.push({ type: 1, file_path: idFrontPath });

      const idBackPath = await uploadSingle(idBack, 2);
      filesPayload.push({ type: 2, file_path: idBackPath });

      for (const uri of degreeImages) {
        const path = await uploadSingle(uri, 3);
        filesPayload.push({ type: 3, file_path: path });
      }

      for (const uri of galleryImages) {
        const path = await uploadSingle(uri, 5);
      ...
      await applyPartnerMutation.mutateAsync({
        ...
        apply_role: 'ktv',
        reviewApplication: {
          agency_id: followAgency && user?.referred_by_user_id ? user.referred_by_user_id : undefined,
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {/* Theo Agency (nếu user có referred_by_user_id) */}
...
const GenderChip: React.FC<GenderChipProps> = ({ label, selected, onPress }) => {
  return (
    <TouchableOpacity
      onPress={onPress}
      className={`flex-1 items-center rounded-full border py-3 ${
        selected ? 'border-primary-color-2 bg-primary-color-2/10' : 'border-gray-300 bg-white'
      }`}>
      <Text
        className={`text-base font-inter-medium ${
          selected ? 'text-primary-color-2' : 'text-gray-700'
        }`}>
        {label}
      </Text>
    </TouchableOpacity>
