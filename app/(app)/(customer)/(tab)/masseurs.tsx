...
import { HeaderApp, KTVCard, KTVCardSkeleton } from '@/components/app/customer';
      ...
      <View className="px-4 pb-1 pt-2 flex-1">
        {isLoading || isRefetching ? (
          Array.from({ length: 6 }).map((_, index) => (
            <KTVCardSkeleton key={`masseur-skeleton-${index}`} />
          ))
        ) : (
          <FlatList
            ...
            ListHeaderComponent={
              <View className="flex-row justify-between items-center gap-1 mb-4">
                <View className="items-center flex-row gap-1">
                  <Text className="text-primary-color-2 font-inter-bold">{pagination?.meta?.total || 0}</Text>
                ...
                {(params?.filter?.category_id && params?.filter?.category_name) &&
                  (
                    ...
                    >
                      ...
                      <Icon as={X} size={10} className={"text-primary-color-2"} />
                    </TouchableOpacity>
                  )
                }
              </View>
            ...
            renderItem={({ item }) => <KTVCard item={item} key={item.id} calculateDistance={calculateDistance} t={t} />}
