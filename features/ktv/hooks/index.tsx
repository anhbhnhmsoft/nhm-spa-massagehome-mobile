...
import {
  useAddServiceMutation, useDeleteServiceMutation,
...
export const useSetService = () => {
  ...
  const deleteService = useSingleTouch((id: string, goBack: boolean = false) => {
    Alert.alert(
      t('ktv.services.delete.confirm.title'),
      t('ktv.services.delete.confirm.message'),
      [
        {
...
export const useFormService = () => {
  ...
  const submit = form.handleSubmit((data) => {
    ...
    if (serviceEdit) {
      // Cập nhật dịch vụ
      updateService({ id: serviceEdit.id, data: formData }, {
        onSuccess: () => {
