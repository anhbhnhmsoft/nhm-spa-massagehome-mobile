...
export function HeaderAppAgency() {
  ...
  return (
    <>
      <GradientBackground
        ...
        <View className="mb-4 mt-2 flex-row items-center justify-between gap-8">
          <View className="flex-row items-center gap-x-3">
            ...
            <View>
              <Text className="text-xs text-white">{t('header_app.hello')}</Text>
              <Text className="font-inter-bold text-lg text-white">{user?.name || 'Stranger'}</Text>
            </View>
          </View>
      ...
      <ModalInfo isVisible={modalInfoVisible} onClose={() => setModalInfoVisible(false)} />
    </>
  );
}
