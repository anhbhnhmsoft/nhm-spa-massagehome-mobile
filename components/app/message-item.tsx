...
const MessageStatus = React.memo(({ status }: { status: PayloadNewMessage['status_sent'] }) => {
...
export const MessageItem = React.memo(
  ({ item, currentUserId, onLongPress, t }: MessageItemProps) => {
  ...
  // Custom comparator – chỉ re-render khi các field hiển thị thay đổi
  (prev, next) =>
    prev.item.id === next.item.id &&
    prev.item.status_sent === next.item.status_sent &&
    prev.item.translated_content === next.item.translated_content &&
